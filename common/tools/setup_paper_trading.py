...
Usage:
    uv run --env-file configs/envs/.env.local python -m common.tools.setup_paper_trading
    uv run --env-file configs/envs/.env.local python -m common.tools.setup_paper_trading --dry-run
"""
...
# Asset IDs — must match inotives_tradings.assets after allowlisting
BTC_ID  = 1
SOL_ID  = 3
USDT_ID = 7
...
async def upsert_strategy(conn, s: dict, venue_id: int, dry_run: bool) -> None:
    existing = await conn.fetchrow(
        "SELECT id, status FROM inotives_tradings.trade_strategies WHERE name = $1 AND deleted_at IS NULL",
        s["name"],
    )
    ...
    if existing:
        ...
        if status != "ACTIVE" and not dry_run:
            await conn.execute(
                "UPDATE inotives_tradings.trade_strategies SET status='ACTIVE', updated_at=NOW() WHERE id=$1",
                strat_id,
            )
...
async def run(dry_run: bool) -> None:
    ...
    try:
        ...
        venue = await conn.fetchrow(
            "SELECT id FROM inotives_tradings.venues WHERE name = $1 AND deleted_at IS NULL",
            VENUE_NAME,
        )
        if venue:
            venue_id = venue["id"]
            ...
            venue_id = await conn.fetchval(
                """
                INSERT INTO inotives_tradings.venues (name, venue_type, source_id, metadata)
