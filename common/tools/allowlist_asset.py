...
it into inotives_tradings.assets and inotives_tradings.asset_source_mappings so it becomes visible to
all pipelines and bots.

Usage:
    # Dry-run (preview only):
    uv run --env-file configs/envs/.env.local python -m common.tools.allowlist_asset \\
...
async def run(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if not cg_source_id:
            print(f"ERROR: data source '{COINGECKO_SOURCE_CODE}' not found in inotives_tradings.data_sources.")
            sys.exit(1)
        ...
        # ── 3. Upsert into inotives_tradings.assets ────────────────────────────────────────
        ...
        print(f"✓ inotives_tradings.assets: id={asset_id}  code={code}  name={name}")
        ...
        print(f"✓ inotives_tradings.asset_source_mappings: {COINGECKO_SOURCE_CODE} → '{coin['coingecko_id']}'")
        ...
        if args.cmc_id and cmc_source_id:
            ...
            print(f"✓ inotives_tradings.asset_source_mappings: {CMC_SOURCE_CODE} → '{args.cmc_id}'")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Allow-list a CoinGecko coin into inotives_tradings.assets for trading/tracking."
    )
    parser.add_argument(
