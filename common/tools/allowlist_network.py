...
inserts it into inotives_tradings.networks and inotives_tradings.network_source_mappings so it becomes
visible to all pipelines and bots.

Usage:
    # Dry-run (preview only):
    uv run --env-file configs/envs/.env.local python -m common.tools.allowlist_network \\
...
async def run(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        # ── 3. Upsert into inotives_tradings.networks ─────────────────────────────────────
        ...
        print(f"✓ inotives_tradings.networks: id={network_id}  code={code}  name={name}  category={category}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Allow-list a CoinGecko platform into inotives_tradings.networks."
    )
    parser.add_argument(
