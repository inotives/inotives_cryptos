"""
Seed base.assets from a CSV file.

Usage:
    uv run --env-file configs/envs/.env.local python scripts/seed_assets_from_csv.py
    uv run --env-file configs/envs/.env.local python scripts/seed_assets_from_csv.py --csv db/seeds/assets_seed_B001.csv
    uv run --env-file configs/envs/.env.local python scripts/seed_assets_from_csv.py --dry-run
...
async def seed(csv_path: Path, dry_run: bool) -> None:
    ...
    try:
        ...
        network_rows = await conn.fetch("SELECT id, code FROM base.networks")
        ...
        asset_rows = await conn.fetch("SELECT id, code FROM base.assets")
        ...
        for r in backing_rows:
            ...
            await conn.execute(
                "UPDATE base.assets SET backing_asset_id = $1 WHERE id = $2",
                backing_id, asset_id,
            )
            updated += 1
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Seed base.assets from CSV.")
