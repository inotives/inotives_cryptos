"""
Seed base.asset_metrics_1d from a CoinMarketCap OHLCV CSV export.
...
Usage:
    ...
    uv run --env-file configs/envs/.env.local python scripts/seed_metrics_1d_from_csv.py \
...
async def seed(csv_path: Path, asset_code: str, source_code: str, dry_run: bool) -> None:
    ...
    try:
        ...
        if not asset_row:
            print(f"ERROR: asset '{asset_code}' not found in base.assets.")
            sys.exit(1)
        ...
        if not source_row:
            print(f"ERROR: source '{source_code}' not found in base.data_sources.")
            sys.exit(1)
        ...
        for r in rows:
            existing = await conn.fetchval(
                """SELECT id FROM base.asset_metrics_1d
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Seed base.asset_metrics_1d from a CMC OHLCV CSV. "
