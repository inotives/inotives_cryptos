"""
Unit tests for bots.pricing_bot.main.

Covers:
  build_watch_pairs — pair string parsing
...
import bots.pricing_bot.main as bot
...
class TestBuildWatchPairs:

    def test_single_pair(self):
        result = bot.build_watch_pairs(["btc/usdt"])
        assert result == [("btc", "usdt", "BTC/USDT")]

    def test_multiple_pairs(self):
        result = bot.build_watch_pairs(["btc/usdt", "eth/usdt"])
        assert len(result) == 2
        assert result[1] == ("eth", "usdt", "ETH/USDT")
...
class TestFetchAndStore:
    ...
    async def test_inserts_row_for_valid_ticker(self):
        ...
        with patch("bots.pricing_bot.main.get_conn", make_get_conn(conn)):
    ...
    @pytest.mark.asyncio
    async def test_skips_symbol_absent_from_response(self):
        exchange = AsyncMock()
        exchange.fetch_tickers = AsyncMock(return_value={
...
class TestLoadIds:
    ...
    async def test_returns_source_id_and_pair_map(self):
        ...
        with patch("bots.pricing_bot.main.get_conn", make_get_conn(conn)):
            source_id, pair_map = await bot.load_ids(
                "exchange:cryptocom", [("btc", "usdt", "BTC/USDT")]
            )
    ...
    async def test_raises_when_source_not_found(self):
        ...
        with patch("bots.pricing_bot.main.get_conn", make_get_conn(conn)):
            with pytest.raises(ValueError, match="not found"):
                await bot.load_ids(
                    "exchange:unknown", [("btc", "usdt", "BTC/USDT")]
                )
    ...
    async def test_skips_pair_when_base_asset_not_in_db(self):
        ...
        conn.fetchrow = AsyncMock(side_effect=[
            ...
            self._make_row(11),  # quote
        ])

        with patch("bots.pricing_bot.main.get_conn", make_get_conn(conn)):
            _, pair_map = await bot.load_ids(
                "exchange:cryptocom", [("btc", "usdt", "BTC/USDT")]
            )
    ...
    @pytest.mark.asyncio
    async def test_partial_pairs_loaded(self):
        conn = make_mock_conn()
        conn.fetchrow = AsyncMock(side_effect=[
            self._make_row(5),   # source
        ...
        watch = [("btc", "usdt", "BTC/USDT"), ("eth", "usdt", "ETH/USDT")]
        with patch("bots.pricing_bot.main.get_conn", make_get_conn(conn)):
            _, pair_map = await bot.load_ids("exchange:cryptocom", watch)
