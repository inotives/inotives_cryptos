...
class TestMarketDataDelegation:
    ...
    @pytest.mark.asyncio
    async def test_fetch_ohlcv_delegates(self, paper, mock_real_exchange):
    ...
    @pytest.mark.asyncio
    async def test_close_delegates(self, paper, mock_real_exchange):
        await paper.close()
...
class TestFetchTradingFees:

    @pytest.mark.asyncio
    async def test_delegates_to_real_exchange(self, paper, mock_real_exchange):
        mock_real_exchange.fetch_trading_fees = AsyncMock(return_value={
            "maker": 0.001, "taker": 0.002,
        })
        result = await paper.fetch_trading_fees("BTC/USDT")
        mock_real_exchange.fetch_trading_fees.assert_awaited_once_with("BTC/USDT")
