...
class DcaGridStrategy(BaseStrategy):
    ...
    async def process(self, exchange: BaseExchangeConnection, strategy: dict, paper: bool = False) -> None:
        ...
        async with get_conn() as conn:
            ...
            # In paper mode, cycles go directly to CLOSED (no CLOSING state).
            if not paper:
            ...
            # In paper mode, fills happen instantly in _place_grid_buy so no polling needed.
            if not paper:
            ...
            levels = await conn.fetch(
                """
                SELECT * FROM inotives_tradings.trade_grid_levels
                WHERE cycle_id = $1
            ...
            # If recent price range exceeds daily ATR, skip placing new levels
            # to avoid filling grid too fast during a volatility spike.
            if pending_levels:
                intraday_hot = await self._is_intraday_volatility_elevated(
                    conn, strategy, indicators_cache=None,
                )
                if intraday_hot:
                    logger.info(
                        "Strategy %d | cycle %d: intraday volatility elevated "
                        "— pausing grid level placement this tick.",
    ...
    async def _open_cycle(
        ...
        async with conn.transaction():
            ...
            await conn.execute(
                """
                INSERT INTO inotives_tradings.trade_dca_cycle_details
            ...
            await conn.executemany(
                """
                INSERT INTO inotives_tradings.trade_grid_levels
    ...
    async def _poll_open_orders(
        ...
        open_levels = await conn.fetch(
            ...
            JOIN inotives_tradings.trade_orders tor ON tor.id = gl.order_id
    ...
    async def _record_fill(
        ...
        async with conn.transaction():
            ...
            await conn.execute(
                """
                INSERT INTO inotives_tradings.trade_executions
            ...
            await conn.execute(
                """
                UPDATE inotives_tradings.trade_orders
                SET status          = 'FILLED',
                    filled_quantity = $1,
                    avg_fill_price  = $2,
                    fee_total       = $3,
    ...
    async def _place_grid_buy(
        ...
        logger.info(
            "Strategy %d | %splacing grid buy level %d: qty=%.8f @ %.6f",
            strategy_id, "[PAPER] " if self._paper else "",
            level["level_num"], quantity, price,
        )

        if self._paper:
            # Paper mode: instant fill, no order/execution records.
            # Mark grid level directly as FILLED.
            await conn.execute(
                """
                UPDATE inotives_tradings.trade_grid_levels
                SET status = 'FILLED', filled_at = NOW(), updated_at = NOW()
    ...
    async def _close_cycle(
        ...
        if self._paper:
            # Paper mode: skip exchange sell, finalize directly.
            await self._finalize_closed_cycle_paper(
                conn, strategy, cycle, filled_levels, trigger,
            )
            return
    ...
    async def _poll_closing_cycles(
        ...
        for cycle in closing_cycles:
            ...
            sell_order = await conn.fetchrow(
                """
                SELECT * FROM inotives_tradings.trade_orders
    ...
    async def _finalize_closed_cycle(
        ...
        buy_summary = await conn.fetchrow(
            ...
            FROM inotives_tradings.trade_executions
        ...
        async with conn.transaction():
            ...
            await conn.execute(
                """
                INSERT INTO inotives_tradings.trade_pnl
                    (cycle_id, strategy_id,
    ...
    async def _finalize_closed_cycle_paper(
        self,
        conn,
        strategy: dict,
        cycle: dict,
        ...
        """
        Paper mode cycle close: compute PnL from grid levels, log it,
        but skip all execution/order/pnl DB writes.

        Still writes: trade_grid_levels (cancel remaining), capital_locks (release),
        trade_cycles (CLOSED), system_events.
        """
        strategy_id = strategy["id"]
        cycle_id    = cycle["id"]

        # ── Compute PnL from grid levels (no trade_executions in paper mode) ──
        total_buy_cost = sum(
            Decimal(str(l["target_price"])) * Decimal(str(l["quantity"]))
            for l in filled_levels
        )
        total_buy_qty = sum(Decimal(str(l["quantity"])) for l in filled_levels)
        avg_buy_price = total_buy_cost / total_buy_qty if total_buy_qty > 0 else Decimal("0")

        # Use latest observed price as simulated sell price
        price_row = await conn.fetchrow(
            """
            SELECT observed_price FROM inotives_tradings.price_observations
            WHERE base_asset_id = $1 AND quote_asset_id = $2
            ORDER BY observed_at DESC LIMIT 1
            """,
            strategy["base_asset_id"], strategy["quote_asset_id"],
        )
        sell_price = Decimal(str(price_row["observed_price"])) if price_row else avg_buy_price
        sell_proceeds = sell_price * total_buy_qty

        maker_fee = Decimal(str(strategy.get("maker_fee_pct", 0.0025)))
        taker_fee = Decimal(str(strategy.get("taker_fee_pct", 0.005)))
        estimated_fees = (total_buy_cost * maker_fee) + (sell_proceeds * taker_fee)
        gross_pnl = sell_proceeds - total_buy_cost
        ...
        async with conn.transaction():
            ...
            await conn.execute(
                """
                UPDATE inotives_tradings.trade_cycles
                SET status = 'CLOSED', close_trigger = $1, closed_at = NOW(), updated_at = NOW()
                WHERE id = $2
                """,
                trigger, cycle_id,
            )
            ...
            await conn.execute(
                ...
                strategy_id, cycle_id,
                (
                    f"[PAPER] Closed DCA grid cycle #{cycle['cycle_number']} "
                    f"({trigger}) | "
                    f"net_pnl={float(net_pnl):.2f} ({float(pnl_pct):.4f}%)"
                ),
                json.dumps({
        ...
        logger.info(
            "Strategy %d | [PAPER] cycle %d CLOSED (%s) | "
            "buy_cost=%.2f sell_proceeds=%.2f gross=%.2f net=%.2f (%.4f%%) fees=%.4f",
            strategy_id, cycle_id, trigger,
            total_buy_cost, sell_proceeds, gross_pnl, net_pnl, pnl_pct, estimated_fees,
        )
    ...
    async def _maybe_retune(
        ...
        async with conn.transaction():
            ...
            await conn.execute(
                """
                UPDATE inotives_tradings.trade_dca_cycle_details
    ...
    async def _is_intraday_volatility_elevated(
        self,
        conn,
        strategy: dict,
        ...
        """
        Check if recent intraday price range exceeds daily ATR.

        Computes (max - min) of price_observations over the last `lookback_hours`
        and compares against the daily ATR(14). Returns True if the intraday range
        exceeds the daily ATR, indicating a volatility spike that should pause
        new grid level placements.
        """
        indicators = indicators_cache or await self._load_latest_indicators(
            conn, strategy["base_asset_id"],
        )
        if not indicators or not indicators.get("atr_14"):
            return False

        daily_atr = float(indicators["atr_14"])

        range_row = await conn.fetchrow(
            """
            SELECT MAX(observed_price) - MIN(observed_price) AS intraday_range
        ...
        if not range_row or range_row["intraday_range"] is None:
            return False

        intraday_range = float(range_row["intraday_range"])

        if intraday_range > daily_atr:
            logger.debug(
                "Intraday volatility guard: range=%.4f > daily_atr=%.4f (%.1f%%)",
                intraday_range, daily_atr,
                (intraday_range / daily_atr * 100) if daily_atr > 0 else 0,
            )
            return True
