...
Each strategy implementation must:
  1. Set `strategy_type` as a class-level string matching the value stored in
     inotives_tradings.trade_strategies.strategy_type (e.g. 'DCA_GRID').
...
class BaseStrategy(ABC):
    ...
    #: Must match inotives_tradings.trade_strategies.strategy_type — used as registry key.
    strategy_type: str
    ...
    async def process(
        ...
        Args:
            ...
            strategy: Full row from inotives_tradings.trade_strategies, with additional
                      fields: base_asset_code, quote_asset_code.
            paper: If True, skip DB writes for execution data (trade_executions,
                   trade_orders, trade_pnl). Positions tracked via grid levels /
                   cycle metadata only.
        """
        ...
