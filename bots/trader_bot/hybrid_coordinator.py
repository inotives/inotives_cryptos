...
async def _circuit_breaker_active(
    ...
    """
    # Get today's opening price: first price observation of the current UTC day.
    # Falls back to the latest completed daily open from asset_metrics_1d if
    # no intraday observations exist yet (e.g. bot just started).
    ...
    if not open_row:
        # Fallback: latest completed daily bar
        open_row = await conn.fetchrow(
            """
            SELECT open_price AS observed_price
    ...
    if not open_row or not open_row["observed_price"]:
        return False
    ...
    daily_open    = float(open_row["observed_price"])
