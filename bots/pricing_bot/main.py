...
and writes observations to inotives_tradings.price_observations.

Usage:
    # Single pair
    uv run --env-file configs/envs/.env.local \\
        python -m bots.pricing_bot.main \\
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="inotives_tradings.data_sources.source_code (default: exchange:cryptocom)",
    )
    p.add_argument(
        "--pair",
        dest="pairs",
    ...
    async with get_conn() as conn:
        ...
        for base_code, quote_code, symbol in watch_pairs:
            base_row = await conn.fetchrow(
                "SELECT id FROM inotives_tradings.assets WHERE LOWER(code) = $1", base_code
            )
            quote_row = await conn.fetchrow(
                "SELECT id FROM inotives_tradings.assets WHERE LOWER(code) = $1", quote_code
            )
...
async def fetch_and_store(exchange, source_id: int, pair_map: dict[str, tuple[int, int]]) -> None:
    ...
    async with get_conn() as conn:
        ...
        await conn.executemany(
            """
            INSERT INTO inotives_tradings.price_observations
