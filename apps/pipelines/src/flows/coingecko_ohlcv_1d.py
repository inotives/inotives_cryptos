"""
CoinGecko daily OHLCV fetch flow.

Fetches yesterday's completed daily candle for all assets mapped to
api:coingecko in base.asset_source_mappings, then upserts into
base.asset_metrics_1d.
...
Schedule: daily at 02:00 UTC (configured in prefect.yaml).
"""
...
# days=90 → 4h candles from /ohlc (aggregated to daily)
OHLCV_DAYS = 90
...
@task(name="load-coingecko-asset-mappings")
async def load_asset_mappings() -> list[dict]:
    """
    Load all assets that have a CoinGecko mapping from base.asset_source_mappings.
    ...
    try:
        rows = await conn.fetch(
            ...
            FROM base.asset_source_mappings asm
            JOIN base.assets       a  ON a.id  = asm.asset_id
            JOIN base.data_sources ds ON ds.id = asm.source_id
...
@task(name="fetch-coingecko-ohlcv", retries=3, retry_delay_seconds=60)
...
@task(name="fetch-coingecko-market-chart", retries=3, retry_delay_seconds=60)
...
@task(name="upsert-ohlcv-metrics")
async def upsert_ohlcv(
    ...
    """
    Upsert a single day's OHLCV + market cap into base.asset_metrics_1d.
    ...
    try:
        ...
        prev_close = await conn.fetchval(
            """
            SELECT close_price FROM base.asset_metrics_1d
        ...
        await conn.execute(
            """
            INSERT INTO base.asset_metrics_1d (
...
@flow(name="coingecko-fetch-ohlcv-1d", log_prints=True)
async def coingecko_fetch_ohlcv_1d_flow(target_date: date | None = None) -> int:
    """
