...
Entry conditions (ALL must pass):
  1. Regime Score >= 61 from base.asset_market_regime (strong trend)
...
class TrendFollowingStrategy(BaseStrategy):
    ...
    async def _maybe_open_cycle(
        ...
        logger.info(
            "Strategy %d: TREND_FOLLOW entry — price=%.4f size=%.6f stop=%.4f | %s",
            strategy_id, fill_price, position_size, initial_stop_loss, reason,
        )
    ...
    async def _open_cycle(
        ...
        async with conn.transaction():
            ...
            await conn.execute(
                ...
                f"Opened TREND_FOLLOW cycle #{cycle_number} for strategy {strategy_id}",
                json.dumps({
    ...
    async def _manage_open_position(
        ...
        # Load ATR for trailing stop calculation
        indicators = await self._load_latest_indicators(conn, strategy["base_asset_id"])
        if not indicators:
    ...
    async def _close_position(
        ...
        async with conn.transaction():
            ...
            await conn.execute(
                ...
                """,
                strategy_id, cycle_id,
                f"Closed TREND_FOLLOW cycle via {trigger} for strategy {strategy_id}",
                json.dumps({
                    "trigger":        trigger,
                    "entry_price":    float(entry_price),
        ...
        logger.info(
            "Strategy %d: TREND_FOLLOW cycle %d CLOSED via %s | "
            "entry=%.4f exit=%.4f pnl=%.2f%%",
            strategy_id, cycle_id, trigger, entry_price, fill_price, pnl_pct,
        )
