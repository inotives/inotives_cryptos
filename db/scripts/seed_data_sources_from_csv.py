"""
Seed inotives_tradings.data_sources from a CSV file via inotives_tradings.upsert_data_source().

Usage:
    uv run --env-file configs/envs/.env.local python db/scripts/seed_data_sources_from_csv.py
    uv run --env-file configs/envs/.env.local python db/scripts/seed_data_sources_from_csv.py --csv db/seeds/data_sources_seeds.csv
    uv run --env-file configs/envs/.env.local python db/scripts/seed_data_sources_from_csv.py --dry-run
...
async def seed(csv_path: Path, dry_run: bool) -> None:
    ...
    try:
        for source_code, provider_name, category, tier_name, rate_limit_rpm, site_url in rows:
            ...
            await conn.fetchval(
                "SELECT inotives_tradings.upsert_data_source($1, $2, $3, $4, $5, $6)",
                source_code, provider_name, category, tier_name, rate_limit_rpm, site_url,
            )
            if exists:
                updated += 1
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Seed inotives_tradings.data_sources from CSV.")
    parser.add_argument(
        "--csv",
        type=Path,
        default=DEFAULT_CSV,
