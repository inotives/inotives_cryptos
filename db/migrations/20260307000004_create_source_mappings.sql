...
CREATE TABLE inotives_tradings.asset_source_mappings (
...
CREATE TABLE inotives_tradings.asset_source_mappings_history (LIKE inotives_tradings.asset_source_mappings INCLUDING DEFAULTS);
ALTER TABLE inotives_tradings.asset_source_mappings_history
    ADD COLUMN changed_at  TIMESTAMPTZ,
    ADD COLUMN changed_by  BIGINT,
    ADD COLUMN change_type TEXT,
    ADD COLUMN changes     JSONB;
CREATE INDEX ON inotives_tradings.asset_source_mappings_history (sys_period);
CREATE INDEX ON inotives_tradings.asset_source_mappings_history (changed_at);
CREATE INDEX ON inotives_tradings.asset_source_mappings_history (changed_by);

CREATE TRIGGER auditing_trigger_asset_source_mappings
    BEFORE INSERT OR UPDATE ON inotives_tradings.asset_source_mappings
...
CREATE TRIGGER soft_delete_trigger_asset_source_mappings
    BEFORE DELETE ON inotives_tradings.asset_source_mappings
...
CREATE TRIGGER versioning_trigger_asset_source_mappings
    BEFORE UPDATE OR DELETE ON inotives_tradings.asset_source_mappings
    FOR EACH ROW EXECUTE PROCEDURE inotives_tradings.versioning('inotives_tradings.asset_source_mappings_history');
...
CREATE TABLE inotives_tradings.network_source_mappings (
    id                BIGSERIAL PRIMARY KEY,
    network_id        BIGINT NOT NULL REFERENCES inotives_tradings.networks(id)      DEFERRABLE INITIALLY DEFERRED,
    source_id         BIGINT NOT NULL REFERENCES inotives_tradings.data_sources(id)  DEFERRABLE INITIALLY DEFERRED,
    source_identifier TEXT   NOT NULL,  -- Primary key the source uses (numeric ID, slug, etc.)
...
CREATE TABLE inotives_tradings.network_source_mappings_history (LIKE inotives_tradings.network_source_mappings INCLUDING DEFAULTS);
ALTER TABLE inotives_tradings.network_source_mappings_history
    ADD COLUMN changed_at  TIMESTAMPTZ,
    ADD COLUMN changed_by  BIGINT,
    ADD COLUMN change_type TEXT,
    ADD COLUMN changes     JSONB;
CREATE INDEX ON inotives_tradings.network_source_mappings_history (sys_period);
CREATE INDEX ON inotives_tradings.network_source_mappings_history (changed_at);
CREATE INDEX ON inotives_tradings.network_source_mappings_history (changed_by);

CREATE TRIGGER auditing_trigger_network_source_mappings
    BEFORE INSERT OR UPDATE ON inotives_tradings.network_source_mappings
    FOR EACH ROW EXECUTE PROCEDURE inotives_tradings.set_audit_fields();

CREATE TRIGGER soft_delete_trigger_network_source_mappings
    BEFORE DELETE ON inotives_tradings.network_source_mappings
    FOR EACH ROW EXECUTE PROCEDURE inotives_tradings.trigger_soft_delete();

CREATE TRIGGER versioning_trigger_network_source_mappings
    BEFORE UPDATE OR DELETE ON inotives_tradings.network_source_mappings
    FOR EACH ROW EXECUTE PROCEDURE inotives_tradings.versioning('inotives_tradings.network_source_mappings_history');


-- migrate:down
DROP TABLE IF EXISTS inotives_tradings.network_source_mappings_history;
DROP TABLE IF EXISTS inotives_tradings.network_source_mappings CASCADE;
DROP TABLE IF EXISTS inotives_tradings.asset_source_mappings_history;
DROP TABLE IF EXISTS inotives_tradings.asset_source_mappings CASCADE;
