...
--
-- Example:
--   SELECT inotives_tradings.upsert_data_source(
--       'exchange:cryptocom', 'Crypto.com', 'MARKET_DATA',
--       'free', 60, 'https://crypto.com', '{}'::jsonb
--   );
CREATE OR REPLACE FUNCTION inotives_tradings.upsert_data_source(
    p_source_code    TEXT,
    p_provider_name  TEXT,
    p_category       TEXT,           -- must match inotives_tradings.data_source_category enum
...
DECLARE
    v_id BIGINT;
BEGIN
    INSERT INTO inotives_tradings.data_sources
        (source_code, provider_name, category, tier_name, rate_limit_rpm, site_url, metadata)
    VALUES
        (p_source_code, p_provider_name, p_category::inotives_tradings.data_source_category,
...
DROP FUNCTION IF EXISTS inotives_tradings.upsert_data_source(TEXT, TEXT, TEXT, TEXT, INTEGER, TEXT, JSONB);
